...
from ..deps import _client_ip, _get_auth, _read_body, _require_admin, check_tenant_quota, db, get_tenant_id, handle_errors
...
async def auth_exchange(request: Request):
    """Redeem a one-time SAML/OIDC exchange code for a NOBA token.
    30-second TTL, single-use. No auth required but rate-limited.
    """
...
async def api_login(request: Request):
    ...
    if user_data and verify_password(user_data[0], password, username=username):
        if users.is_disabled(username):
            db.audit_log("login_blocked", username, "account disabled", ip)
...
async def api_social_callback(provider: str, request: Request):
    ...
    try:
        ...
        # Route through sso-callback — same popup flow as SAML
        oidc_code = secrets.token_urlsafe(32)
        with _saml_exchange_codes_lock:
            _saml_exchange_codes[oidc_code] = (noba_token, time.time() + 30)
        import urllib.parse as _up
        return RedirectResponse(f"/api/saml/complete?code={_up.quote(oidc_code)}")
...
async def api_profile_password(request: Request, auth=Depends(_get_auth)):
    ...
    if db.password_history_check(tenant_id, username, new_hash):
    ...
    db.password_history_add(tenant_id, username, new_hash)
...
async def api_users_post(request: Request, auth=Depends(_require_admin)):
    ...
    if action == "change_password":
        target = body.get("username", "").strip()
        pw     = body.get("password", "")
        target_tenant = db.get_user_tenant(target) or "default"
        pw_err = check_password_strength(pw, tenant_id=target_tenant)
        if pw_err:
            raise HTTPException(400, pw_err)
        new_hash = pbkdf2_hash(pw)
        if db.password_history_check(target_tenant, target, new_hash):
            raise HTTPException(400, "Password was recently used. Choose a different password.")
        if not users.update_password(target, new_hash):
            raise HTTPException(404, "User not found")
        db.password_history_add(target_tenant, target, new_hash)
...
@handle_errors
def api_keys_list(auth=Depends(_require_admin), tenant_id: str = Depends(get_tenant_id)):
    return db.list_api_keys(tenant_id=tenant_id)


@router.post("/api/admin/api-keys")
@handle_errors
async def api_keys_create(request: Request, auth=Depends(_require_admin), tenant_id: str = Depends(get_tenant_id)):
    import hashlib
    username, _ = auth
    body = await _read_body(request)
    ...
    check_tenant_quota(tenant_id, "API key", "max_api_keys")
    ...
    if isinstance(allowed_ips, list):
        import json as _json
        allowed_ips_str = _json.dumps(allowed_ips)
    else:
        allowed_ips_str = "[]"
    rate_limit = int(body.get("rate_limit") or 0)
    db.insert_api_key(key_id, name, key_hash, role, expires_at,
                      scope=scope, allowed_ips=allowed_ips_str, rate_limit=rate_limit,
                      tenant_id=tenant_id)
    ...
    return {"id": key_id, "key": raw_key, "name": name, "role": role,
            "scope": scope, "allowed_ips": allowed_ips, "rate_limit": rate_limit}
