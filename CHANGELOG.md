...
### Fixed
- **Social login configs never saved** — `socialProviders` and `oidcProviderName` were missing from `WEB_KEYS`. Settings UI could not persist Google/GitHub/Facebook/Microsoft OAuth configs.
- **Duplicate `GET /api/admin/sessions` route** — Two identical route definitions in `auth.py`; removed the duplicate.
- **`/api/workflow-nodes` missing error handler** — Added `@handle_errors` decorator to prevent stack trace leaks.
...
### Security
- **OIDC discovery TLS verification** — Added OIDC `.well-known/openid-configuration` discovery with `verify=True` default. Configurable via `oidcVerifySsl` toggle for self-signed IdPs.
- **Social provider secrets encrypted at rest** — `socialProviders.*.clientSecret` values are now Fernet-encrypted on write, decrypted on read, masked to `***` on API GET, and redacted in audit logs.
- **Security contact updated** — Removed dead `security@noba.app` email; GitHub private security advisories is now the sole disclosure channel.
