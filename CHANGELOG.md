...
### Fixed
- **Dashboard masonry grid stability** — Cards no longer jump during live SSE updates. Watch `activeInstances` via stable string key; ResizeObserver change-guard prevents cascade; HardwareCard GPU Temp uses once-seen ref pattern.
- **Dashboard save/load card order** — Save captures live drag order from `_sortable.toArray()`; load stores `cardOrder` inside `preferences.preferences` (persists to server); `activeInstances` watcher re-applies order for late SSE-driven managed instance cards.

### Enterprise (enterprise-v2 branch)
- **SAML 2.0 SSO** — IdP-initiated and SP-initiated SAML login flows (`/api/saml/login`, `/api/saml/acs`, `/api/saml/metadata`). SLO (single logout) via `/api/saml/slo`. Config via `SAML_IDP_METADATA_URL` or inline XML. Certificate rotation supported via multiple IdP certs. Integrated with `UserStore.update_role()` for JIT role assignment from SAML attributes.
- **WebAuthn / passkey MFA** — FIDO2 credential registration and authentication (`/api/webauthn/register/begin|complete`, `/api/webauthn/authenticate/begin|complete`). TOTP-style backup codes (`/api/webauthn/backup-codes`). Credentials stored in `webauthn_credentials` table with `UNIQUE(credential_id)` constraint.
- **SCIM 2.0 provisioning** — Full RFC 7644 User CRUD at `/api/scim/v2/Users`, with `ServiceProviderConfig`, `Schemas`, and `ResourceTypes` discovery endpoints. Bearer token auth via single-active-token design (`scim_tokens` table, 1-year TTL). Admin endpoint to rotate token at `POST /api/admin/scim-token`. Role mapping from SCIM `roles` attribute.
- **API key scoping** — Per-key `scope` (path prefix list), `allowed_ips` (CIDR allowlist), and `rate_limit` (requests/minute). Enforced in `_get_auth` for `ApiKey ` prefix. In-memory rolling rate-limit window with `threading.Lock`-protected dict. Returns HTTP 403 for scope/IP violations, HTTP 429 for rate limit.
- **Structured JSON logging** — `server/logging_config.py` exposes `setup_logging()`. Set `NOBA_LOG_FORMAT=json` for newline-delimited JSON (`timestamp`, `level`, `logger`, `message`, optional `exception`). `NOBA_LOG_LEVEL` controls verbosity. Integrates with log aggregators (Loki, Graylog, CloudWatch Logs).
- **PostgreSQL pluggable backend** — Set `DATABASE_URL=postgresql://...` to switch from SQLite to PostgreSQL with no code changes. `PgConnectionAdapter` in `db/postgres_adapter.py` transparently translates `?→%s`, `BLOB→BYTEA`, `REAL→DOUBLE PRECISION`, `INTEGER PRIMARY KEY AUTOINCREMENT→SERIAL PRIMARY KEY`, `INSERT OR REPLACE→ON CONFLICT DO UPDATE`, `INSERT OR IGNORE→ON CONFLICT DO NOTHING`, and `cursor.lastrowid` via `RETURNING id`. Zero changes to 26 domain modules or 22 mixins. `psycopg2-binary` is an optional dependency loaded lazily (listed in `requirements-enterprise.txt`).
- **PostgreSQL read connection pool** — `ThreadedConnectionPool` (`NOBA_PG_POOL_MIN`/`NOBA_PG_POOL_MAX`, default 1–10) for concurrent reads. Each thread gets a dedicated checkout from the pool. Write connection remains single and serialized by `threading.RLock`.
- **SQLite → PostgreSQL migration** — `scripts/migrate-to-postgres.py` copies all data from an existing SQLite database to PostgreSQL using `INSERT … ON CONFLICT DO NOTHING` (idempotent, re-runnable). Usage: `DATABASE_URL=postgresql://... python3 scripts/migrate-to-postgres.py [sqlite_path]`.
- **`@pytest.mark.postgres` test suite** — `tests/test_postgres_backend.py` covers schema init, domain roundtrips (metrics, preferences, heal_ledger, incidents, network devices), transaction rollback, and WAL checkpoint no-op. Automatically skipped when `DATABASE_URL` is not set.
