...
if command -v get_config &>/dev/null; then
    ...
    # get_config returns strings, convert to proper booleans
...
# -------------------------------------------------------------------
show_version() {
    echo "motd-generator.sh version 1.0.1"
    exit 0
}
...
print_updates() {
    ...
    if command -v dnf &>/dev/null; then
        updates=$(timeout "$timeout_val" dnf check-update -q 2>/dev/null | wc -l || echo 0)
    ...
    if command -v flatpak &>/dev/null; then
        flatpak_updates=$(timeout "$timeout_val" flatpak remote-ls --updates 2>/dev/null | wc -l || echo 0)
    ...
    [ "$any_updates" = false ] && echo "  All packages are up to date."
}
...
print_quote() {
    ...
    if [ -f "$QUOTE_FILE" ]; then
        ...
        quote=$(curl -s --max-time 2 "https://api.quotable.io/random" 2>/dev/null | jq -r '.content + " – " + .author' 2>/dev/null || true)
